...
namespace raumPlayer.ViewModels
{
    ...
    {
        public ElementContainer(DIDLContainer didl) :base(didl)
        {
        ...
        private async  void OnImageOpened(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (bitmap.UriSource == new Uri("ms-appx:///Assets/disc_gray.png", UriKind.Absolute))
                {
                    ...
                    AverageColorImageArt = new SolidColorBrush(Colors.Transparent);
                }
                else
                {
                    ...
                    using (var stream = buffer.AsStream().AsRandomAccessStream())
                    {
                        ...
                        //ImageArt = bitmap;
                        AverageColorImageArt = new SolidColorBrush(Color.FromArgb((byte)127, (byte)avgR, (byte)avgG, (byte)avgB));
                    }
                }
            }
            catch (Exception)
